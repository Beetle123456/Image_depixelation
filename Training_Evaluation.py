...
def training(
    ...
    for epochs in range(num_epochs):    # iterate over the number of epochs
        batch_loss = []     # to store each batch loss
        ...
        for inputs, known_array, targets in train_loader:    # iterate over the train_loader for each individual batch
            ...
            batch_loss.append(loss.item())  # appends the loss per each batch to the list
        train_loss.append(sum(batch_loss)/len(batch_loss))  # summing up each individual batch loss and divide it by
        ...
        for inputs, known_array, targets in eval_loader:
            ...
            for batch in output:
                for img in batch:
                    ...
                    break
            """
        eval_loss.append(sum(batch_loss)/len(batch_loss))
